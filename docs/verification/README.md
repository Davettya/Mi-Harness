...
最新增量：2026-09-17 Fake-IP 修复后，模型/策略/API/平台配置定向回归 **44 项通过**（含 23 项新增边界用例）。本机仍解析到 `198.18.0.88`，但已授权官方模型 HTTPS 请求可通过策略与 TLS 并到达认证层；未使用用户 Key。[回归日志](fake-ip-regression.log)、[实际网络证据](fake-ip-live.json)。此前模型设置修订的完整历史结果见下文；本次没有重复整套全量测试。

日期：2026-09-17。对象：Local Agent Harness 0.1.0 的 P0/P1 本地实现，Windows / Python 3.11.9。用户已指定“先完成本地验收，真实服务稍后配置”。
